...
void BM_gemm_bf16p_bf16p_bf16_4vlxvl(benchmark::State& state) {
...
BENCHMARK(BM_gemm_bf16p_bf16p_bf16_4vlxvl)
    ->Args({128, 128, 128})
    ->Args({1024, 1024, 1024})
    ->Args({4096, 4096, 4096})
    ->Args({128, 128, 16384})
...
void BM_gemm_bf16p_bf16p_bf16_2vlx2vl(benchmark::State& state) {
  const size_t M = static_cast<size_t>(state.range(0));
  const size_t N = static_cast<size_t>(state.range(1));
  const size_t K = static_cast<size_t>(state.range(2));
...
BENCHMARK(BM_gemm_bf16p_bf16p_bf16_2vlx2vl)
    ->Args({128, 128, 128})
    ->Args({1024, 1024, 1024})
    ->Args({4096, 4096, 4096})
    ->Args({128, 128, 16384})
...
void gemm_bf16p_bf16p_bf16_mt(const sme::GemmParams& p,
                             const void* lhs_packed, const void* rhs_packed,
                             __bf16* out, size_t num_threads) {
  auto pack = sme::gemm_bf16_4vlxvl_packing_params();
  ...
  for (size_t t = 0; t < num_threads; t++) {
    ...
    // LHS packed: tiles in M order, each tile is K * m_tile * sizeof(bf16).
    const size_t lhs_offset_bytes =
        (m_start / m_tile) * p.K * m_tile * sizeof(__bf16);
    const auto* lhs_slice =
        static_cast<const char*>(lhs_packed) + lhs_offset_bytes;

    __bf16* out_slice = out + m_start * p.N;

    threads.emplace_back([=] {
      sme::GemmParams sp{m_count, p.N, p.K};
      sme::gemm_bf16p_bf16p_bf16_4vlxvl(sp, lhs_slice, rhs_packed, out_slice);
    });
  }
...
void BM_gemm_bf16p_bf16p_bf16_mt(benchmark::State& state) {
  const size_t M = static_cast<size_t>(state.range(0));
  const size_t N = static_cast<size_t>(state.range(1));
  const size_t K = static_cast<size_t>(state.range(2));
  ...
  for (auto _ : state) {
    gemm_bf16p_bf16p_bf16_mt(p, lhs_packed.get(), rhs_packed.get(), C.data(),
                            num_threads);
    benchmark::DoNotOptimize(C.data());
    benchmark::ClobberMemory();
  }
...
BENCHMARK(BM_gemm_bf16p_bf16p_bf16_mt)
